...
def generate_interactive_html_report(json_response: dict, original_findings: list) -> str:
    ...
    <body>
    <div class="container">
        <div class="header">📊 Code Review Report</div>
    """
    ...
    for file, issues in file_findings.items():
        ...
        medium = len([i for i in issues if i.get("severity", "").upper()=="MEDIUM"])
        ...
        <div class="file-header" id="header-file{count}" onclick="toggleFile('file{count}')">
            ...
            <div class="priority-badge {badge_class}">
                <span class="priority-dot dot-{label.split()[0].lower()}"></span>
...
if __name__ == "__main__":
    try:
        ...
        with open("review_output.json","w") as f: json.dump({
            "full_review": filtered,
            "criticals": criticals,
            "file": FILE_TO_REVIEW,
